---
title: 'U-PASS: a unified power analysis of association tests'
author: "GAO Zheng"
date: "April 5, 2019"
...
In association tests for categorical phenotypes and genetic variants, counts of subjects in each phenotype-genetic variant combination are tabulated in the form of a contingency tables.
For a 2-genetic-variant-by-2-phenotype definition, we have the following table of counts.
...
kable(dt, "html", escape = FALSE, align = "c") %>%
  ...
  add_header_above(c(" " = 1, "Genotypes" = 2, " " = 1))
```
...
Power analysis starts by assuming an alternative, typically described by a disease model (recessive, dominant, additive, etc.).
...
## Model-invariant parametrization {#subsec:model-invariant}
...
```{r echo=FALSE}
dt <- data.frame(matrix(c("Cases", "$\\mu_{11}$", "$\\mu_{12}$", "$\\phi$",
                          "Controls", "$\\mu_{21}$", "$\\mu_{22}$", "$1-\\phi$"), byrow = T, nrow = 2))
...
* the marginal distribution of phenotypes, i.e., fraction of Cases $\phi$,

* the conditional distribution of risk variant among Controls, i.e., risk allele frequency (RAF) in the Control group 
$$
    f := \mu_{21}/(1-\phi),
$$
* and the odds ratio (OR) of the genotype Variant 1 to Variant 2
$$
...
Power may be calculated by directly prescribing the trio $(\phi, f, R)$, and the sample size.
...
colnames(dt) <- c("Cond. Prob.", "Variant 1", "Variant 2", "Counts by phenotype")
...
This Fisher exact test conditions on the number of observations of both the phenotype variants and genetic variants, leading to a hypergeometric distribution of the first cell count $O_{11}$ given the marginals $n_1$, $n_2$, and $O_{11}+O_{21}$, under the null hypothesis.
...
## Test-independent power analysis {#subsec:power-calculation}
...
\begin{equation} \label{eq:alternative-multinomial}
    \sqrt{n}(\mu - \mu^{(0)}) \rightarrow \delta \begin{pmatrix} 1 & -1 \\ -1 & 1 \end{pmatrix},
\end{equation}
...
In 2-by-2 contingency tables, under the assumption that counts in the contingency table follow the multinomial distributions, 
...
Proof of Theorem 1 is detailed in [Section 3](#sec:proof).

Theorem 1 is the central result that paves way for a unified powers analysis.
...
If tests are based on allele type counts, the effective sample sizes should be doubled, and the appropriate non-centrality parameter becomes  $\delta = \sqrt{2n}(\theta - f)(1-\phi)$.
...
For correctly calibrated tests, an alternative hypothesis with expected variant counts less than the minimum calibration numbers should have power close to zero; asymptotic power approximations do not apply for theses alternatives.
...
We conduct an extensive simulation study to examine the quality of this finite-sample correction [in the next Secction](#sec:numerical).
...
See, e.g., @ferguson2017course Chapter 10 and Chapter 24, and @lehmann2004elements Chapter 5; see also, @hunter2002 for an accessible derivation of the formula \eqref{eq:non-centrality}.

To see the asymptotic equivalence with the LR statistic under logistic regressions with binary predictors, we reparametrize the LR statistic as
\begin{equation*}
    \mu_{11}^{O_{11}}\mu_{12}^{O_{12}}\mu_{21}^{O_{21}}(1-\mu_{11}-\mu_{12}-\mu_{21})^{O_{22}} = \phi^{n_1}(1-\phi)^{n_2}p_{1}^{O_{11}}(1-p_{1})^{O_{12}}p_{2}^{O_{21}}(1-p_{2})^{O_{22}}
\end{equation*}
...
Therefore, the terms involving $\phi$ cancels in the LR statistic, and the LR statistic coincides with the logistic regressions likelihood ratio, where $p_1$, $p_2$ are further reparametrized as
\begin{align*}
...
\begin{align}
    ...
    &\sim \sqrt{(\theta(1-\theta)) \Big/ (\phi(1-\phi))}. \label{eq:center-sd}
\end{align}
Dividing \eqref{eq:center-mean} by \eqref{eq:center-sd}, we conclude that the centers of the distribution of $t$ converges to 
$$\delta\Big/\sqrt{\theta(1-\theta)\phi(1-\phi)},$$
which is precisely the square root of the non-centrality in \eqref{eq:non-centrality}.
...
We examine the accuracy of the asymptotic approximations in Theorem 1 in finite samples, and of the correction by minimum calibration number detailed in [Section 2](#sec:finite-sample), via numerical simulation.
